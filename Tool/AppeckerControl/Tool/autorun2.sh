...
function waitForAppStart()
{
	...
	while ! appOnLine
	do
		echo "App is not yet running, waiting...."
		sleep 1
	done
	echo "App started!"
}
...
function runSingleCase()
{
	...
	$TOOL_DIR/argopen ${APP_ID} -caseid "$caseID" -logPath /Appecker 1>/dev/null 2>&1
...
function runInMonkeyMode()
{
	...
	cleanUpMonkeyMode
	$TOOL_DIR/argopen ${APP_ID} -caseid "$MONKEY_CASE_ID" 1>/dev/null 2>&1
