...
- (int)launchApp:(NSString *)path withFamily:(NSString *)family
    ...
    if (!exitOnStartup && stderrPath == nil) {
        fcntl(STDERR_FILENO, F_GETPATH, &fdPath);
        stderrPath = [NSString stringWithUTF8String:fdPath];
    }
    [config setSimulatedApplicationStdErrPath:stderrPath];
    
    if (!exitOnStartup && stdoutPath == nil) {
        fcntl(STDOUT_FILENO, F_GETPATH, &fdPath);
        stdoutPath = [NSString stringWithUTF8String:fdPath];
    }
